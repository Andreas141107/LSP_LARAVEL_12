    ...
    <x-sidebar>
        <div class="bg-white w-full min-h-screen border px-6 py-4 rounded-md shadow-md flex flex-col">
            ...
            <div class="flex justify-between items-center mb-6 flex-shrink-0">
                <h1 class="text-2xl font-bold text-blue-600">Data Menu</h1>
                <a href="{{ route('menu.tambah') }}" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 transition font-semibold">
                    + Tambah Menu
                </a>
            </div>
                        ...
                        @else
                            @foreach ($menu as $mn)
                                <tr class="border-t hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 border">{{ $mn->nama }}</td>
                                    <td class="px-6 py-4 border">{{ $mn->harga }}</td>
                                    <td class="px-6 py-4 space-x-3 border">
                                        <div class="flex justify-center space-x-4">
                                        <a href="{{ route('edit.menu',['id'=>$mn->id]) }}" class="text-blue-500 font-medium hover:underline">Edit</a>
                                        <form action="{{ route('hapus.menu',['id'=>$mn->id]) }}" id="delete-form-{{ $mn->id }}" method="POST">
